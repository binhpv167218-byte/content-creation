...
**Dấu câu bắt buộc:** Trong toàn bộ nội dung bài post và text trên hình ảnh, CHỈ được dùng: `, . : ; ? !` — KHÔNG dùng bất kỳ loại dấu gạch nào (`—` em dash hay `-` hyphen). Áp dụng mọi nơi: post.md (phần Post Text), content.json, --text argument của add-photo-overlay.py, carousel slide, infographic prompt. Metadata nội bộ (Lịch đăng, Format...) không áp dụng.
...
- **Theo phương pháp:** 5 viral replication (carousel) + 5 trend surfing (ảnh cá nhân)
...
#### Phương Pháp Tạo Nội Dung Theo Loại Post (BẮT BUỘC)

Mỗi loại post có phương pháp chủ đạo và hỗ trợ riêng. Áp dụng đúng khi tạo nội dung — không hoán đổi giữa các loại.

| Loại post                                  | Phương pháp chủ đạo | Hỗ trợ 1           | Hỗ trợ 2            | Độ dài                       |
| ------------------------------------------ | ------------------- | ------------------ | ------------------- | ---------------------------- |
| **Carousel**                               | Viral Replication   | List Content       | Comparison          | 5-7 slides (giữ quy tắc sẵn) |
| **Post + Hình cá nhân**                    | Trend Surfing       | Storytelling       | Q&A                 | Giữ quy tắc chữ hiện tại     |
| **Video Market/Dự án**                     | Trend Surfing       | Data Visualization | News                | 60s (giữ)                    |
| **Video Vlog Daily**                       | Viral Replication   | Storytelling       | —                   | 60s (giữ)                    |
| **Video Community (AI + Truyền cảm hứng)** | How-to              | Storytelling       | Case Study + Quotes | **120-180s** (tăng từ 60s)   |

---

#### Carousel — 2 bài/tuần (BMN, Thứ 2 + Thứ 6)

**Phương pháp: Viral Replication** (tìm bài viral → lấy đóng gói → thay substance bằng góc nhìn Bình)
...
**Phương pháp: Trend Surfing** (bám trend/câu hỏi đang nổi trong niche BĐS/môi giới → áp góc nhìn cá nhân Bình vào). KHÔNG dùng Viral Replication cho loại bài này.
...
#### Video Cộng Đồng (Video AI + Truyền Cảm Hứng)

**Phương pháp: How-to** (xác định vấn đề cụ thể → giải thích từng bước rõ ràng → kết hợp storytelling cá nhân). Kênh: Kênh cá nhân, TikTok mới, IG, Threads, YT Shorts.

Storytelling cá nhân về hành trình, AI workflow, góc nhìn nghề. Tệp mục tiêu: đồng nghiệp môi giới, sales BĐS cùng định hướng — không phải khách mua nhà. **Độ dài: 120-180s** (không phải 60s như Market/Vlog).

#### Video Thị Trường

**Phương pháp: Trend Surfing** (bắt đầu bằng tin tức/trend thị trường đang được quan tâm → dẫn dắt bằng data). Kênh: BMN, TikTok hiện tại. **Độ dài: 60-75s.**

Thông tin, phân tích, dữ liệu về thị trường BĐS Đà Nẵng và các dự án. Tệp mục tiêu: nhà đầu tư, người mua nhà.

#### Vlog

**Phương pháp: Viral Replication** (lấy format vlog viral → kể chuyện thật hậu trường). Kênh: Kênh cá nhân, IG, Threads. **Độ dài: 60s.**
...
| `inbox.md`               | Links và content Bình paste vào để lấy ý tưởng          | Liên tục — thêm bất cứ lúc nào, xử lý qua `/analyze-inbox` |
...
- **BẮT BUỘC trước khi tạo bất kỳ batch content nào** → đọc `inbox.md`, lồng ghép entries PENDING chưa [USED] vào brief nếu relevant với chủ đề batch
...
1. **Viral Replication Ideas** — tìm các bài đã chứng minh hiệu quả và đề xuất cách lấy phần đóng gói (dùng cho Carousel, Vlog)
2. **Trend Surfing Ideas** — bám các thứ đang nổi lên ngay lúc này trong niche (dùng cho Ảnh cá nhân, Video Market)
3. **How-to Ideas** — xác định vấn đề thực tế, giải thích bước rõ ràng (dùng cho Video Community)
...
**Phương pháp:** Viral Replication (Carousel/Vlog) / Trend Surfing (Ảnh cá nhân/Video Market) / How-to (Video Community)
...
| **Gemini 2.5 Flash** | Phân tích video local MP4 + YouTube URL qua Gemini API — tự động, không cần nhắc | `GEMINI_API_KEY` trong `.env`       |
...
### Quy Tắc Dùng Gemini Video (TỰ ĐỘNG — Không Cần User Nhắc)

Khi user đưa video hoặc YouTube link để phân tích, **Claude PHẢI tự chạy `analyze-video.js`** — không hỏi, không chờ nhắc:

| Input          | Ví dụ                            | Lệnh                                       |
| -------------- | -------------------------------- | ------------------------------------------ |
| File MP4 local | Footage Đà Nẵng, video dự án     | `node analyze-video.js <path> "<câu hỏi>"` |
| YouTube URL    | Video đối thủ, content tham khảo | `node analyze-video.js <url> "<câu hỏi>"`  |

**Workflow bắt buộc:**

1. User đưa video path hoặc YouTube URL + yêu cầu phân tích → Claude tự chạy `node -e "import('./analyze-video.js').then(m => m.analyzeVideo('<input>', '<task>')).then(console.log)"` qua Bash
2. Đọc output → tóm tắt, phân tích, đề xuất theo yêu cầu
3. Không yêu cầu user phải nói "dùng Gemini" — tự xử lý

**Giới hạn:** Facebook video URL không hỗ trợ (cần auth). Chỉ YouTube và file local MP4.

### Quy Tắc Lưu Inbox (Khi User Yêu Cầu)

Khi user paste link hoặc nội dung **kèm lệnh "lưu vào kho nội dung"** (hoặc tương tự: "lưu vào inbox", "thêm vào kho"), **Claude phân tích và lưu vào `context/intelligence/inbox.md`**:

| Input                                              | Detect bằng            | Tool                                                                              |
| -------------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------- |
| URL báo (vnexpress, cafef, tuoitre, batdongsan...) | Domain trong URL       | Perplexity: `python3 scripts/research_bot.py "URL" --output /tmp/inbox_result.md` |
| YouTube URL                                        | youtube.com / youtu.be | Gemini: `analyze-video.js`                                                        |
| TikTok URL                                         | tiktok.com             | Apify: `clockworks/free-tiktok-scraper`                                           |
| Raw text / ghi chú                                 | Không có URL           | Claude phân tích trực tiếp                                                        |

**Workflow:**

1. User paste link/nội dung + nói "lưu vào kho nội dung"
2. Chạy tool phù hợp → tạo entry chuẩn (format xem `.claude/commands/analyze-inbox.md`)
3. Append entry vào đúng section trong `context/intelligence/inbox.md`
4. Báo ngắn: "Đã lưu vào inbox: [tên entry ngắn]"
